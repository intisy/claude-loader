...
if (firstArg === "auth" && args[1] === "login") {
  ...
  if (providers.length === 0) {
    const wantInstall = await tuiConfirm({ 
      title: "[cc] No auth providers found. Install claude-code-auth?",
      defaultYes: true 
    });
    if (wantInstall) {
      console.log("[cc] Installing claude-code-auth via plugin-updater...");
      try {
        const updaterModule = await import("plugin-updater");
        const updater = updaterModule.default || updaterModule;
        updater.updatePlugin("claude-code-auth", "https://github.com/intisy/claude-code-auth.git");
        updater.deployToExecutionDir("claude-code-auth", join(homedir(), ".config", "claude", "plugin"));
        console.log("[cc] Successfully installed. Please run 'cc auth login' again.");
      } catch (e) {
        console.error("[cc] Failed to install claude-code-auth. Please ensure plugin-updater is installed.", e);
      }
    } else {
      console.log("[cc] No providers available. Exiting.");
    }
    process.exit(0);
  }

  let chosen;
  const authItems = providers.map(p => ({ label: p.name, detail: `from ${p.plugin}` }));
  const idx = await tuiSelect(authItems, { title: "[cc] Select an auth provider:" });
  if (idx < 0) {
    console.log("[cc] Cancelled.");
    process.exit(0);
  }
  ...
  if (!existsSync(chosen.loginScript)) {
    console.log(`[cc] Login script not found: ${chosen.loginScript}`);
    process.exit(1);
  }
...
if (firstArg === "auth" && args[1] === "reset") {
  console.log("[cc] Note: Rate limit resetting is now handled by provider plugins directly.");
  process.exit(0);
}
// @ts-nocheck
